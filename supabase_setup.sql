...
create policy "public submits cert" on cert_requests for insert to anon with check (status = 'pending');
create policy "admin manages cert" on cert_requests for all to authenticated using (true) with check (true);

-- 5) WhatsApp group join requests
create table group_requests (
  id bigint generated always as identity primary key,
  name text not null,
  phone text not null,
  scope text not null default 'street' check (scope in ('street','building')),
  street int check (street between 1 and 61),
  bldg text default '—',
...
create policy "public submits group req" on group_requests for insert to anon with check (status = 'pending');
create policy "admin manages group req" on group_requests for all to authenticated using (true) with check (true);

-- 5) Problem reports (tracked with status + timestamps)
...
-- NOTE: if you already created the reports table before, run this once to add the new column:
